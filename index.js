...
client.on("ready", () => {
  client.guilds.cache.forEach((guild) => {
    guild.commands.create({
      name: "warn",
      description:
        "Aggiunge un warn specifico ad un utente per un tempo limitato",
      options: [
        {
          name: "membro",
    ...
    guild.commands.create({
      name: "fineassistenza",
      description:
        "Usa il comando quando avrai terminato un'assistenza (vocale)",
    });
...
setInterval(function () {
  ...
  if (scheduledTime.includes(current) && current !== lastSentTime) {
    ...
    let embed = new Discord.EmbedBuilder()
      .setTitle("Server On")
      .setDescription(
        "<:IRP:1294760298951081995>Ricordo che il server è on<:IRP:1294760298951081995>\nEntrate sulla mappa Roblox e godetevi l'RP.\n"
      )
      .setColor("Green")
...
client.on("guildMemberAdd", async (member) => {
  ...
  console.log(
    "Ruolo Cittadino di Italian RP aggiunto a " + member.user.username
  );
});
...
setInterval(() => {
  ...
  fs.writeFileSync(
    path,
    JSON.stringify({ userRoles: updatedRoles }, null, 2),
    "utf8"
  );
}, 60000);
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId == "buttoncreate") {
    ...
    let rowmodal = new Discord.ActionRowBuilder().addComponents(problemInput);
    ...
    removedRoles.forEach((removedRole) => {
      updatedRoles = updatedRoles.filter((entry) => {
        if (entry.roleId === removedRole.id && entry.userId === newMember.id) {
          console.log(
            `Ruolo manualmente rimosso: ${removedRole.name} (${removedRole.id}) dall'utente ${newMember.user.tag}`
          );
  ...
  if (!oldTimeout && newTimeout) {
    ...
    if (timeoutroles) {
      ...
      if (target.id === newMember.id) {
        ...
        const duration =
          durationMinutes.toString() == "1"
            ? `${durationMinutes} minuto`
...
client.on("guildMemberRemove", async (member) => {
  try {
    ...
    if (!kickLog)
      return console.log(
        `${member.user.tag} ha lasciato il server (non espulso)`
      );
    ...
    console.error(
      `Errore nel gestire la rimozione di ${member.user.tag}:`,
      error
    );
  }
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName == "warn") {
    try {
      ...
      if (interaction.channel.id !== "1287008127140499508")
        return interaction.reply({
          content: "Il warn va assegnato in <#1287008127140499508>",
          ephemeral: true,
        });

      if (!allowedRoles.includes(role.id)) {
        return interaction.reply({
          content: "Questo ruolo non è di tipo warn",
          ephemeral: true,
        });
      }
      ...
      if (!member.roles.cache.has("1276959088047034490")) {
        ...
        client.channels.cache
          .get("1286783677291958352")
          .send({ embeds: [embed2] });
      }
    } catch (err) {
      console.error(err);
      return interaction.reply({
        content: "Impossibile assegnare il warn a questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "unwarn") {
    try {
      ...
      if (interaction.channel.id !== "1287008127140499508")
        return interaction.reply({
          content: "Il warn va tolto in <#1287008127140499508>",
          ephemeral: true,
        });
      ...
      if (!member.roles.cache.has(role.id)) {
        return interaction.reply({
          content: "Il membro deve ancora avere il warn",
          ephemeral: true,
        });
      }
      ...
      console.error(err);
      return interaction.reply({
        content: "Impossibile togliere il warn a questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "convoca") {
    try {
      ...
      if (interaction.channel.id !== "1276965741035388948") {
        return interaction.reply({
          content: "**Puoi convocare utenti solo in <#1276965741035388948>**",
          ephemeral: true,
        });
      }
      ...
      if (!channel) {
        return interaction.reply({
          content: "Non sei in un canale vocale",
          ephemeral: true,
        });
      }
      ...
      if (channel) {
        ...
        if (!assistenza.includes(channel.id)) {
          return interaction.reply({
            content:
              "**Sei in un canale vocale, ma non in un canale vocale per assistenza**",
            ephemeral: true,
          });
        }
        ...
        let targetvoiceChannel = guild.members.cache.get(member.user.id).voice
          .channel;

        if (interaction.member.user.id == member.user.id) {
          return interaction.reply({
            content: `**Non puoi convocare te stesso.**`,
            ephemeral: true,
          });
        }
        ...
        if (targetvoiceChannel && targetvoiceChannel.id === channel.id) {
          return interaction.reply({
            content: `<@${member.user.id}> **è già in assistenza con te.**`,
            ephemeral: true,
          });
        }
        ...
        if (amount < 0 || amount > 10) {
          return interaction.reply({
            content: `La quantità ha un minimo di 1 e un massimo di 10`,
            ephemeral: true,
          });
        }
        ...
        if (amount > 1) {
          for (let i = 0; i < amount; i++) {
            interaction.channel.send({
              content: `<@${member.user.id}> **CONVOCATO IN** <#${channel.id}>`,
              allowedMentions: { users: [`${member.user.id}`] },
            });

            if (i == amount - 1) {
              await interaction.reply({
                content: `Inviate ${amount} convocazioni`,
                ephemeral: true,
              });
              break;
            }
      ...
      console.error(err);
      return interaction.reply({
        content: "Impossibile convocare questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "clear") {
    ...
    if (amount < 1 || amount > 100) {
      return interaction.reply({
        content: "Per favore fornisci un numero valido tra 1 e 100.",
        ephemeral: true,
      });
    }
    ...
    if (!interaction.member.roles.cache.has("1276959088047034490")) {
      return interaction.reply({
        content: "Non hai il permesso di eliminare messaggi.",
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      await interaction.followUp({
        content: `${amount} messaggi sono stati eliminati.`,
        ephemeral: true,
      });
  ...
  if (interaction.commandName == "kick") {
    try {
      ...
      if (!member.kickable) {
        return interaction.reply({
          content: "Non puoi espellere membri importanti",
          ephemeral: true,
        });
      }

      if (
        member.roles.cache.has("1277980184695406633") ||
        member.roles.cache.has("1277978978371502090")
      ) {
        return interaction.reply({
          content: "Non puoi espellere membri importanti dello staff",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        ifbotcommand.set("moderatorid", interaction.user.id);
        interaction.reply({
          content: `Hai espulso ${member.user.username} dal server`,
          ephemeral: true,
        });
      } catch (error) {
      ...
      console.error(err);
      return interaction.reply({
        content: "Impossibile espellere questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "ban") {
    try {
      let member = interaction.options.getMember("membro");
      let reasonban =
        interaction.options.getString("motivo") || "Nessun motivo";
      ...
      if (!member.bannable) {
        return interaction.reply({
          content: "Non puoi bannare membri importanti",
          ephemeral: true,
        });
      }
      ...
      if (
        ...
        return interaction.reply({
          content: "Non puoi bannare membri importanti dello staff",
          ephemeral: true,
        });
      }
      ...
      try {
        ...
        ifbotcommand.set("moderatorid", interaction.user.id);
        interaction.reply({
          content: `Hai bannato ${member.user.username} dal server`,
          ephemeral: true,
        });
      } catch (error) {
        console.error(`Errore durante il ban: ${error}`);
        interaction.reply({
          content: "Si è verificato un errore durante il ban del membro.",
          ephemeral: true,
        });
      }
    } catch (err) {
      console.error(err);
      return interaction.reply({
        content: "Impossibile bannare questo membro",
      });
    }
  }

  // TODO: Change dadada with actual server link
    ...
    let embed = new Discord.EmbedBuilder()
      ...
      .addFields(
        ...
        },
        {
          name: "Staff:",
          value: `${
            server.roles.cache.get("1276959088047034490").members.size
          }`,
          inline: true,
        },
        {
  ...
  if (interaction.commandName == "fineassistenza") {
    ...
    if (interaction.channel.id !== "1278333489695162379") {
      return interaction.reply({
        content: "Non puoi usare questo comando qui",
        ephemeral: true,
      });
    }
    ...
    let row = new Discord.ActionRowBuilder().addComponents(input1);

    let row2 = new Discord.ActionRowBuilder().addComponents(input2);

    let row3 = new Discord.ActionRowBuilder().addComponents(input3);

    let row4 = new Discord.ActionRowBuilder().addComponents(input4);
  ...
  if (interaction.commandName == "creazione") {
    ...
    if (!interaction.member.roles.cache.has("1289649734818074746")) {
      return interaction.reply({
        content: "Non sei un developer",
        ephemeral: true,
      });
    }
  ...
  if (interaction.commandName == "blacklist") {
    if (interaction.channel.id !== "1297308966161743872") {
      return interaction.reply({
        content: "Non puoi usare questo comando qui",
        ephemeral: true,
      });
  ...
  if (interaction.commandName == "timeout") {
    try {
      ...
      if (!member.moderatable) {
        return interaction.reply({
          content: "Impossibile mettere in timeout questo utente",
          ephemeral: true,
        });
      }
      ...
      if (member.communicationDisabledUntilTimestamp) {
        return interaction.reply({
          content: "Il membro è già in timeout",
          ephemeral: true,
        });
      }
      ...
      return interaction.reply({
        content: `Hai messo in timeout con successo <@${member.user.id}> per ${duration} minuti.`,
        ephemeral: true,
      });
    } catch (err) {
      console.error(err);
      return interaction.reply({
        content: "Impossibile impostare il timeout a questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "untimeout") {
    try {
      ...
      if (!member.communicationDisabledUntilTimestamp) {
        return interaction.reply({
          content: "Il membro non è in timeout",
          ephemeral: true,
        });
      }
      ...
      return interaction.reply({
        content: `Hai tolto dal timeout con successo <@${member.user.id}>`,
        ephemeral: true,
      });
    } catch (err) {
      console.error(err);
      return interaction.reply({
        content: "Impossibile togliere il timeout a questo membro",
      });
    }
  }
  ...
  if (interaction.commandName == "github") {
    interaction.reply({
      content: "https://github.com/PixelDarks/ItalianRP-Bot",
      ephemeral: true,
    });
  }
  ...
  if (interaction.commandName == "assistenzaon") {
    ...
    if (interaction.channel.id !== chatass)
      return interaction.reply({
        content: "Non puoi usare questo comando qui",
        ephemeral: true,
      });
    ...
    let embed = new Discord.EmbedBuilder()
      ...
      .setColor("Green")
      .setDescription(
        `Entrate in <#1276968449574174720> se volete avere qualche informazione, oppure per chiedere informazioni o spoilers riguardante il server`
      );

    interaction.reply({
      embeds: [embed],
      content: "<@&1276923411498795100>",
      allowedMentions: { roles: ["1276923411498795100"] },
    });
  }
  ...
  if (interaction.commandName == "assistenzaoff") {
    ...
    let embed = new Discord.EmbedBuilder()
      ...
      .setColor("Red")
      .setDescription(
        `Tornate la prossima volta per chiedere qualche informazione`
      );
  ...
  if (interaction.commandName == "nickname") {
    try {
      ...
      if (!user.moderatable) {
        return interaction.reply({
          content: "Impossibile impostare un nickname a questo utente",
          ephemeral: true,
        });
      }
      ...
      if (nick.length > 32) {
        return interaction.reply({
          content: "Il nick deve essere di 32 caratteri o meno",
          ephemeral: true,
        });
      }
      ...
      interaction.reply({
        content: `<@${user.user.id}> è stato aggiornato con successo`,
        ephemeral: true,
      });
    } catch (err) {
      console.error(err);
  ...
  if (interaction.commandName == "mute") {
    try {
      ...
      if (user.roles.cache.has("1318999628519309352"))
        return interaction.reply({
          content: "Questo utente è già mutato",
          ephemeral: true,
        });
      ...
      interaction.reply({
        content: "Utente mutato con successo",
        ephemeral: true,
      });
    } catch (err) {
      console.error(err);
  ...
  if (interaction.commandName == "unmute") {
    try {
      ...
      if (!user.roles.cache.has("1318999628519309352"))
        return interaction.reply({
          content: "Questo utente non è ancora stato mutato",
          ephemeral: true,
        });
      ...
      interaction.reply({
        content: "Utente smutato con successo",
        ephemeral: true,
      });
    } catch (err) {
      console.error(err);
...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isModalSubmit()) {
    if (interaction.customId === "modalTicket") {
      ...
      if (
        server.channels.cache.find(
          (canale) => canale.topic == `User ID: ${user.id}`
        )
      ) {
      ...
      server.channels
        ...
        .then((canale) => {
          ...
          if (interaction.member.roles.cache.has("1297128036197470251")) {
            ...
            canale
              .send({
                content: "<@&1289639280473280624>",
                allowedMentions: { roles: ["1289639280473280624"] },
              })
              .then((message) => {
                setTimeout(() => {
                  message
    ...
    if (interaction.customId === "modalcreazioni") {
      ...
      await interaction.reply({
        content:
          "Creazione ricevuta! Controlla i DM per continuare l'operazione",
        ephemeral: true,
      });
      try {
        const dmChannel = await interaction.user.createDM();
        await dmChannel.send(
          "Perfetto! Ora allega un'immagine che vuoi includere nel tuo messaggio."
        );
        ...
        collector.on("end", (collected, reason) => {
          if (reason === "time") {
            interaction.followUp({
              content:
                "Non hai allegato un'immagine in tempo. Operazione annullata.",
              ephemeral: true,
            });
          }
        });
        ...
        await interaction.reply({
          content:
            "Non sono riuscito a inviarti un messaggio privato. Assicurati di avere i DM aperti.",
          ephemeral: true,
        });
      }
    }
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.customId === "deletebutton") {
    if (!interaction.member.roles.cache.has("1289639280473280624")) {
      interaction.reply({
        content: "Il bottone è riservato agli staff",
        ephemeral: true,
        components: [],
        embeds: [],
      });
  ...
  if (interaction.customId === "transbutton") {
    ...
    messages.reverse().forEach((msg) => {
      ...
      if (msg.embeds[0]) {
        ...
        if (msg.embeds[0].description)
          chatLog += `Description: ${msg.embeds[0].description}\n`;
        if (msg.embeds[0].fields[0])
          chatLog += `Fields: ${msg.embeds[0].fields
            .map((x) => `${x.value}`)
            .join(", ")}\n`;
      }

      if (msg.attachments.size > 0)
        chatLog += `Files: ${msg.attachments
          .map((x) => `${x.name} (${x.url})`)
          .join(", ")}\n`;

      if (msg.stickers.size > 0)
        chatLog += `Stickers: ${msg.stickers
          .map((x) => `${x.name} (${x.url})`)
          .join(", ")}\n`;
    ...
    let attachment = new Discord.AttachmentBuilder(
      Buffer.from(chatLog, "utf-8"),
      {
        name: `chatLog-channel-${interaction.channel.id}.txt`,
...
client.on("interactionCreate", (interaction) => {
  ...
  if (interaction.commandName == "add") {
    ...
    if (!topic) {
      interaction.reply({
        content: "Non puoi utilizzare questo comando qui",
        ephemeral: true,
      });
      return;
    }
    ...
    if (topic.startsWith("User ID:")) {
      if (interaction.member.roles.cache.has("1289639280473280624")) {
        try {
          ...
          if (!utente) {
            interaction.reply({
              content: "Inserire un utente valido",
              ephemeral: true,
            });
            return;
          }

          const haspermissionobtained = interaction.channel
            .permissionsFor(utente.user)
            ?.has("ViewChannel");

          if (haspermissionobtained) {
            interaction.reply({
              content: "Questo utente ha già accesso al ticket",
              ephemeral: true,
            });
            return;
          }
          ...
          let ownerticket = server.members.cache.find(
            (m) => m.user.username === interaction.channel.name
          );
          ...
          interaction.reply({
            content: `${utente.toString()} è stato aggiunto al ticket`,
          });
        } catch (err) {
          console.error(err);
          return interaction.reply({
            content: "Impossibile aggiungere al ticket questo membro",
          });
        }
      } else {
        interaction.reply({
          content: "Questo comando è riservato agli staff",
          ephemeral: true,
        });
        return;
      }
    ...
    if (topic.startsWith("User ID:")) {
      if (interaction.member.roles.cache.has("1289639280473280624")) {
        try {
          ...
          if (!haspermissionobtained) {
            interaction.reply(
              "Questo utente non ha ancora l'accesso al ticket"
            );
            return;
          }
          ...
          interaction.reply({
            content: `${utente.toString()} è stato rimosso dal ticket`,
          });
        } catch (err) {
          console.error(err);
          return interaction.reply({
            content: "Impossibile rimuovere questo membro dal ticket",
          });
        }
      } else {
        interaction.reply({
